...
export function projectActivityPayload(
  ...
  const projectedEdit = asRecord(data.edit);
  const projectedEditPath = asTrimmedString(projectedEdit?.path);
  const projectedEditPatch =
    typeof projectedEdit?.patch === "string" && projectedEdit.patch.trim().length > 0
      ? projectedEdit.patch
      : null;
  const isOpenCodeEditActivity =
    toolName?.toLowerCase() === "edit" ||
    (projectedEditPath !== null && projectedEditPatch !== null);
  ...
  const editInputPath = isOpenCodeEditActivity
    ? (asTrimmedString(input?.filePath) ??
      asTrimmedString(editFileDiff?.file) ??
      projectedEditPath ??
      "")
    : "";
  ...
  const editPatch =
    isOpenCodeEditActivity && statusProjectedPayload.status === "completed"
      ? (metadataEditPatch ?? fileEditPatch ?? projectedEditPatch)
      : null;
