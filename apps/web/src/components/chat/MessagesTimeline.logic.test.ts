...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("keeps a tool group after the terminal response visible when the turn is folded", () => {
    ...
    expect(rows.map((row) => row.id)).toEqual([
      "turn-fold:turn-1",
      "assistant-final-entry",
      "work-group:work-entry-after-text-0:details",
  ...
  it("renders one tool call directly after collapsing its lifecycle updates", () => {
    ...
    const completedRow = rows.find((row) => row.id === "command-completed");
    expect(completedRow).toMatchObject({
      kind: "work",
  ...
  it("keeps each integration source and icon in an individually readable group", () => {
    const chromeSource = {
  ...
  it.each([
    ...
    expect(
      rows.flatMap((row) =>
        row.kind === "work" ? row.groupedEntries.map((entry) => entry.toolLifecycleStatus) : [],
      ),
    ).toEqual(statuses);
  });
