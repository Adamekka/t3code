  ...
  return (
    <div className="min-w-0 py-0.5">
      <div className="flex min-w-0 items-center gap-1">
        <button
          type="button"
          className="flex min-h-6 min-w-0 flex-1 cursor-pointer items-center gap-1.5 rounded-md px-0.5 py-0.5 text-left text-[12px] leading-5 transition-colors duration-150 hover:bg-accent/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring/70"
      ...
      {expanded ? <TodoChecklist items={checklistItems} className="mt-0.5 pl-8" /> : null}
    </div>
  );
});
...
const PlainWorkEntryRow = memo(function PlainWorkEntryRow(props: {
  ...
  const canExpand = hasEditDiff || expandedBody !== null || (workEntry.todoItems?.length ?? 0) > 0;
