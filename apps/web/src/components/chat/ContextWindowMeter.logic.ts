...
export function hasAvailableCompactionProvider(input: {
  ...
  const lockedContinuationGroupKey = input.lockedInstanceId
    ...
    : undefined;
  const compatibleProviders = driverProviders.filter(
    (provider) =>
      !lockedContinuationGroupKey || provider.continuationGroupKey === lockedContinuationGroupKey,
  );
  const resolvedProvider = resolveSelectableProviderInstanceEntry(
    compatibleProviders,
    input.instanceId ?? undefined,
  );

  return (
    resolvedProvider?.snapshot.slashCommands.some((command) => command.name === "compact") ?? false
  );
}
